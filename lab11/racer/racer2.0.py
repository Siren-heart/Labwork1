...
class Enemy(pygame.sprite.Sprite):
    ...
    def move(self):
        # --- ДОБАВЛЕНО ДЛЯ 11 НЕДЕЛИ: Увеличение скорости врага ---
        global SCORE 
        # Базовая скорость 7. Увеличивается на 1 за каждые 5 заработанных очков (монет)
        self.speed = 7 + (SCORE // 5)
        # ------------------------------------------------------------
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # --- ДОБАВЛЕНО ДЛЯ 11 НЕДЕЛИ: Разный вес монет ---
        # Выбираем случайный вес для монетки: 1, 2 или 5 очков (с разными шансами)
        self.weight = random.choices([1, 2, 5], weights=[70, 20, 10])[0]
        
        # В зависимости от веса меняем цвет монеты, чтобы игрок их различал
        if self.weight == 1:
            coin_color = GOLD            # Обычная (+1)
        elif self.weight == 2:
            coin_color = (0, 255, 0)     # Зеленая (+2)
        else:
            coin_color = (255, 0, 255)   # Пурпурная (+5)
            
        pygame.draw.circle(self.image, coin_color, (15, 15), 15)
        # --------------------------------------------------
...
while True:
    ...
    # --- ИЗМЕНЕНО ДЛЯ 11 НЕДЕЛИ: Получаем список собранных монет, чтобы узнать их вес ---
    collected_coins = pygame.sprite.spritecollide(P1, coins_group, True)
    for coin in collected_coins:
        SCORE += coin.weight # Добавляем не просто +1, а вес конкретной монетки
        new_coin = Coin()
        C1 = new_coin
        coins_group.add(C1)
        all_sprites.add(C1)
